  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.code === 'ERR_NETWORK') {
        login({ id: 'demo', username: formData.username || 'Alex', email: 'demo@debatex.ai' }, 'demo-token');
        navigate('/dashboard');
      } else {
    ...
    <div className="min-h-screen bg-[#05050A] flex items-center justify-center relative overflow-hidden px-4">
      ...
      >
        ...
        <div className="glass-panel p-8 bg-[#0C0C14]/80 border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.8)] backdrop-blur-2xl rounded-3xl">
          <h2 className="text-2xl font-bold font-display text-white mb-2 text-center">Welcome Back</h2>
          <p className="text-gray-400 text-sm text-center mb-8">Enter the arena and resume your debates.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  type="text" 
                  className="w-full bg-black/50 border border-white/5 rounded-xl py-3.5 pl-12 pr-4 text-white placeholder-gray-600 focus:outline-none focus:border-primary/50 focus:bg-black/80 transition-all text-sm"
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full py-4 mt-4 bg-gradient-to-r from-primary to-blue-500 text-black font-black text-sm uppercase tracking-widest rounded-xl hover:opacity-90 transition-all shadow-[0_0_20px_rgba(0,240,255,0.3)] hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? <><Loader2 size={16} className="animate-spin" /> Authenticating...</> : <>Enter System <ArrowRight size={16}/></>}
            </button>
          </form>
